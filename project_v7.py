...
def init_pascal():
  global pascal
  K = 4
  pascal = [ [1 for _ in range(31) ] for _ in range(LIMIT_HI[K]+1) ]
  n,k = 1,1
  while k!=31:
    if n == LIMIT_HI[K]+1: k,n = k+1,1
    else:
      pascal[n][k] = pascal[n-1][k] + pascal[n][k-1]
      n+=1

def binsearch(ans,sK,K=INF,isTab=False):
  limits = [0,10**15]
  k,ok = sK,True
  lo,hi = k*2,LIMIT_HI[k]
  while k<=K and k<(m>>1) and ok:
    ok = C(hi,k,isTab) >= m
